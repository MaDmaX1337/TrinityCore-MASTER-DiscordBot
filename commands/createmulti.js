  ...
  DMonly: true,
	execute(message, args) {
        ...
        for(let counter = 1, myError = false; counter <= amount && myError === false; counter++){
          connection.query('select count(email) from account where reg_mail = ?', [message.author.id], (error, results, fields) => {

            if (error) return message.reply('An error occured.')
            console.log(Object.values(results[0])[0])
            if (Object.values(results[0])[0] <= 25) {
              try {
              soap.Soap(`bnetaccount create ${username}${counter} ${password}`)
              .then(result => { 
                console.log(result)
                if(result.faultString) {
                  ...
                  embed.addField(counter + ". eMail | Password ", username + counter + " | " + password, false)
                  connection.query(`UPDATE account set reg_mail = '${message.author.id}' WHERE email = '${username}${counter}'`, (error, results, fields) => {
